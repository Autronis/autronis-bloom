          ...
          <ScrollReveal className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 auto-rows-fr">
            {tx.problems.map((p, i) => { const PIcon = problemIcons[i]; return (
              <ScrollRevealItem key={p.title} className="h-full">
                <GlowCard className="rounded-xl border border-border bg-card p-4 sm:p-6 h-full" isAnyHovered={problemHovered !== null} isHovered={problemHovered === i} onHover={() => setProblemHovered(i)} onLeave={() => setProblemHovered(null)}>
          ...
          <ScrollReveal className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-8 sm:mb-12 auto-rows-fr">
            {tx.solutions.map((s, i) => { const SIcon = solutionIcons[i]; return (
              <ScrollRevealItem key={s.title} className="h-full">
                <GlowCard className="rounded-xl border border-border bg-card p-4 sm:p-6 h-full" isAnyHovered={solutionHovered !== null} isHovered={solutionHovered === i} onHover={() => setSolutionHovered(i)} onLeave={() => setSolutionHovered(null)}>
                  <div className="flex items-center gap-2.5 sm:gap-3 mb-2 sm:mb-3">
