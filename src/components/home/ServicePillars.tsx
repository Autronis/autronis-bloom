           ...
           <div className={`flex flex-col ${isEven ? "md:flex-row" : "md:flex-row-reverse"} items-start md:items-center gap-0`}>
            ...
            <div className="flex-1 relative overflow-hidden bg-card flex items-center justify-center min-h-[200px] md:min-h-0 aspect-[4/3] md:aspect-auto md:self-stretch">
              <motion.img
                src={s.image}
                alt={s.title}
                className="absolute inset-0 w-full h-full object-contain z-[1] scale-[1.5] dark:mix-blend-screen dark:invert-0 dark:brightness-[0.85] dark:opacity-85 invert hue-rotate-180 mix-blend-multiply brightness-100 opacity-80"
                animate={
                  hoveredIndex === i
                    ? { scale: 1.55 }
                    : { scale: 1.5 }
