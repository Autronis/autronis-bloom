...
const text = {
  en: {
    ...
    reasons: [
      { title: "Architecture before automation", description: "We don't start with tools, but with structure. First understand how processes and systems work together, then build." },
      { title: "Measurable impact", description: "We define KPIs upfront and only build what demonstrably adds value. Every automation has a clear purpose." },
      { title: "Direct with the builders", description: "You work directly with the engineers. No account managers, no intermediaries, no miscommunication." },
      { title: "Ownership & control", description: "Full documentation and transferability. Custom configurations and developed code remain under your control, without vendor lock-in." },
    ],
  },
  nl: {
    label: "Waarom Autronis",
    title: "Van chaos naar controle.",
    ctaBook: "Plan een kennismaking",
    ctaTeam: "Ontmoet ons team",
    reasons: [
      { title: "Architectuur vóór automatisering", description: "We beginnen niet met tools, maar met structuur. Eerst begrijpen hoe processen en systemen samenwerken, dan bouwen." },
      { title: "Meetbare impact", description: "We definiëren KPI's vooraf en bouwen alleen wat aantoonbaar waarde toevoegt. Elke automatisering heeft een helder doel." },
      { title: "Direct met de bouwers", description: "Je werkt direct met de engineers. Geen accountmanagers, geen tussenpersonen, geen miscommunicatie." },
      { title: "Eigenaarschap & controle", description: "Volledige documentatie en overdraagbaarheid. Custom configuraties en ontwikkelde code blijven onder jouw controle, zonder vendor lock-in." },
    ],
  },
};

const reasonIcons = [Blocks, BarChart3, Users, ShieldCheck];

const ReasonCard = ({ reason, index, hoveredIndex, onHover, onLeave, canHover, icon: Icon }: {
  reason: { title: string; description: string }; index: number; hoveredIndex: number | null;
  onHover: () => void; onLeave: () => void; canHover: boolean; icon: React.ElementType;
}) => {
  const isHovered = canHover && hoveredIndex === index;
  const isAnyHovered = canHover && hoveredIndex !== null;

  return (
    <div onMouseEnter={canHover ? onHover : undefined} onMouseLeave={canHover ? onLeave : undefined}
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-2.5 sm:gap-3 mb-2 sm:mb-3">
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-primary/10 flex items-center justify-center text-primary shrink-0"><Icon size={16} /></div>
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <ScrollReveal className="text-center max-w-2xl mx-auto mb-6 sm:mb-16">
          <ScrollRevealItem>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4">{tx.title}</h2>
          </ScrollRevealItem>
        </ScrollReveal>
        <ScrollReveal>
          <ScrollRevealItem>
