      ...
      <div className="group/card rounded-xl border border-border bg-gradient-to-br from-primary/[0.06] to-card transition-all duration-200 ease-out overflow-hidden" style={{ borderColor: isHovered ? "hsl(var(--primary) / 0.5)" : undefined, boxShadow: isHovered ? "0 0 20px hsl(174 78% 41% / 0.12)" : "none" }} onMouseEnter={canHover ? () => setHoveredIndex(i) : undefined} onMouseLeave={canHover ? () => setHoveredIndex(null) : undefined}>
        <div className={`flex flex-col ${isEven ? "md:flex-row" : "md:flex-row-reverse"} items-start md:items-center gap-0`}>
          ...
          <div className="flex-1 relative overflow-hidden bg-transparent flex items-center justify-center min-h-[180px] md:min-h-0 aspect-[4/3] md:aspect-auto md:self-stretch">
            ...
            <div className="absolute inset-0 z-[3] pointer-events-none" style={{ background: isEven ? "linear-gradient(to right, hsl(var(--card)) 0%, transparent 20%)" : "linear-gradient(to left, hsl(var(--card)) 0%, transparent 20%)" }} />
            <div className="absolute inset-0 z-[3] pointer-events-none" style={{ background: "linear-gradient(to bottom, hsl(var(--card)) 0%, transparent 6%, transparent 94%, hsl(var(--card)) 100%)" }} />
          </div>
        </div>
      </div>
