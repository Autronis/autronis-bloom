      ...
      <div id={`case-${index}`} className="rounded-xl border border-border bg-card overflow-hidden scroll-mt-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 border-t border-border">
          ...
          <div className="lg:col-span-2 border-t lg:border-t-0 border-border p-5 sm:p-6 space-y-5">
            <div><SectionHeader>{t.resultsImpact}</SectionHeader><div className="space-y-1.5">{cs.results.map((r, j) => <div key={j} className="flex items-start gap-2 text-[13px] text-muted-foreground leading-relaxed"><CheckCircle2 size={13} className="text-primary mt-0.5 shrink-0" />{r}</div>)}</div></div>
